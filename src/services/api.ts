...
const YCAPI = {
  ...
  persistence: {
    ...
    create: function (data, callback) {
      ...
      xhr.open(
        "POST",
        YCAPI.persistence.param.baseAddress.concat("/c/s/no-ac"),
        true
      );
    ...
    read: function (filter, callback) {
      ...
      xhr.open(
        "POST",
        YCAPI.persistence.param.baseAddress.concat("/q/s/no-ac"),
        true
      );
      xhr.setRequestHeader("Content-Type", "application/json");
      xhr.setRequestHeader("X-Tenant-Id", YCAPI.param.tenant.id);
  ...
  account: {
    ...
    create: function (account, callback) {
      ...
      xhr.open(
        "POST",
        YCAPI.account.param.baseAddress.concat("/open/account"),
        true
      );
      xhr.setRequestHeader("Content-Type", "application/json");
      xhr.onreadystatechange = function () {
    ...
    get: {
      ...
      byJWT: function (callback) {
        ...
        xhr.open(
          "GET",
          YCAPI.account.param.baseAddress.concat("/account/by/jwt"),
          true
        );
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.setRequestHeader(
          "Authorization",
          "Bearer ".concat(YCAPI.param.app.account.jwt)
        );
        xhr.onreadystatechange = function () {
          if (xhr.readyState === 4) {
            callback({
      ...
      byUsername: function (username, callback) {
        ...
        xhr.open(
          "GET",
          YCAPI.account.param.baseAddress
            .concat("/open/account/by/username/")
            .concat(username)
            .concat("/exists"),
          true
        );
  ...
  role: {
    ...
    create: function (account, callback) {
      ...
      xhr.setRequestHeader(
        "Authorization",
        "Bearer ".concat(YCAPI.param.account.jwt)
      );
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
          callback({
    ...
    get: {
      ...
      byNameAndOrganization: function (name, organizationUuid, callback) {
        ...
        xhr.open(
          ...
          YCAPI.account.param.baseAddress
            .concat("/role/by/name/")
            .concat(name)
            .concat("/organization-uuid/")
            .concat(organizationUuid),
          true
        );
        xhr.setRequestHeader(
          "Authorization",
      ...
      byOrganization: function (organizationUuid, callback) {
        ...
        xhr.open(
          "GET",
          YCAPI.account.param.baseAddress
            .concat("/role/by/organization-uuid/")
            .concat(organizationUuid),
          true
        );
        xhr.setRequestHeader(
    ...
    associate: function (accountRole, callback) {
      ...
      xhr.open(
        "POST",
        YCAPI.account.param.baseAddress.concat("/account-role/using/authority"),
        true
      );
      xhr.setRequestHeader("Content-Type", "application/json");
      xhr.setRequestHeader(
  ...
  filer: {
    ...
    upload: function (fileName, body, callback) {
      ...
      xhr.open(
        "POST",
        YCAPI.filer.param.baseAddress
          .concat("/storage/object/name/")
          .concat(fileName),
        true
      );
      xhr.setRequestHeader("X-Tenant-Id", YCAPI.param.tenant.id);
    ...
    list: function (callback) {
      ...
      xhr.open(
        "GET",
        YCAPI.filer.param.baseAddress.concat("/storage/object"),
        true
      );
      xhr.setRequestHeader("X-Tenant-Id", YCAPI.param.tenant.id);
      // choose either X-API-Master-Key header or Authorization header.
  ...
  auth: {
    ...
    match: function (credentials, callback) {
      ...
      xhr.open(
        "POST",
        YCAPI.auth.param.baseAddress.concat("/internal/signin"),
        true
      );
      xhr.setRequestHeader("Content-Type", "application/json");
      xhr.onreadystatechange = function () {
