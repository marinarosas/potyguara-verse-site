...
export function StoreHighlight() {
  ...
  return (
    <Splide
      className="flex justify-center items-start border-2 border-yellow-500 rounded-2xl"
      ref={splideRef}
      options={{
        rewind: false,
        direction: 'ltr',
    ...
    >
      {highlights.map((highlight) => (
        <SplideSlide
          key={highlight.image}
          className="app-carousel flex flex-col items-center h-[30rem] overflow-hidden rounded-2xl"
        >
          <Image src={highlight.image} alt="" className="rounded-2xl" />
          <div className="absolute flex bottom-8 left-0 justify-between w-full items-center pr-8">
            <div className="px-10 py-4 bg-gradient-to-r from-background to-transparent">
              <h1 className="text-6xl text-primary dark:text-secondary">
                {highlight.title}
              </h1>
            ...
            <Button
              variant={varianteButton}
              className="h-14 w-fit text-xl text-background"
            >
